	...
	else
	{
		...
		for (int i = 0; i < MAX_PLAYERS; ++i)
		{
			if (GET_PLAYER((PlayerTypes)i).isAlive())
			{
				...
				{
					if(GET_TEAM((TeamTypes)GET_PLAYER((PlayerTypes)i).getTeam()).getAtWarCount() > 0)
					{
						someoneInWar = true;
					}
				}
		...
		TurnTimerTypes eTurnTimer = GC.getInitCore().getTurnTimer();
		FAssertMsg(eTurnTimer >= 0 && eTurnTimer < GC.getNumTurnTimerInfos(), "Invalid TurnTimer Selection in InitCore");
		int cityBonus =(GC.getTurnTimerInfo(eTurnTimer).getCityBonus());
		int unitBonus =(GC.getTurnTimerInfo(eTurnTimer).getUnitBonus());
		int baseBonus =(GC.getTurnTimerInfo(eTurnTimer).getBaseTime());
		if(baseBonus + (cityBonus*iMaxCities) + (unitBonus*iMaxUnits) > 1150)
		{
			if(someoneInWar)
			{
				return ( 1600 );
		...
		return ( baseBonus + (cityBonus*iMaxCities) + (unitBonus*iMaxUnits) );
	}
}
...
void CvGame::setPausePlayer(PlayerTypes eNewValue)
{
	...
	if(GC.getLogging())
	{
		...
		char* buffer2 = new char[5000];
		sprintf(buffer2, "%ls PAUSE.txt", GC.getInitCore().getGameName().GetCString());
		TCHAR szOut[1024];
		if(player.isHuman())
		{
			if(eNewValue == 0)
			{
				...
				gDLL->logMsg((TCHAR*)buffer2,szOut);
			}
		}
		if(eNewValue == -1)
		{
			sprintf(szOut, "## UNPAUSE ##\n");
			gDLL->logMsg((TCHAR*)buffer2,szOut);
		}
	}
	//
