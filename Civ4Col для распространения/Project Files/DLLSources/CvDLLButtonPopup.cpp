...
void CvDLLButtonPopup::OnOkClicked(CvPopup* pPopup, PopupReturn *pPopupReturn, CvPopupInfo &info)
{
	...
	case BUTTONPOPUP_MAIN_MENU:
		...
		{	// exit to desktop
			...
			sprintf(szOut, "C:\\Users\\%s\\Documents\\My Games\\Civ4Col\\Saves\\multi\\%ls %ls Ñ%ls ÂûõîäÍàÐÑ.ColonizationSave", usernameBuffer, GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getName(), GC.getLeaderHeadInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getLeaderType()).getDescription(), szYearStr.GetCString());
			CvString nameSave = CvString((const char*)szOut);
			gDLL->getEngineIFace()->SaveGame(nameSave, SAVEGAME_AUTO);
			//SAVE
		...
		{	// exit to main menu
			...
			GAMETEXT.setTimeStr(szYearStr, GC.getGameINLINE().getGameTurn(), true);
			sprintf(szOut, "C:\\Users\\%s\\Documents\\My Games\\Civ4Col\\Saves\\multi\\%ls %ls Ñ%ls ÂûõîäÂÃÌ.ColonizationSave", usernameBuffer, GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getName(), GC.getLeaderHeadInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getLeaderType()).getDescription(), szYearStr.GetCString());
			CvString nameSave = CvString((const char*)szOut);
			gDLL->getEngineIFace()->SaveGame(nameSave, SAVEGAME_AUTO);
			//SAVE
			CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CONFIRM_MENU);
		...
		{	// save game
			...
			*/
			...
			GAMETEXT.setTimeStr(szYearStr, GC.getGameINLINE().getGameTurn(), true);
			sprintf(szOut, "C:\\Users\\%s\\Documents\\My Games\\Civ4Col\\Saves\\multi\\%ls %ls Ñ%ls Ñîõð.ColonizationSave", usernameBuffer, GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getName(), GC.getLeaderHeadInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getLeaderType()).getDescription(), szYearStr.GetCString());
			CvString nameSave = CvString((const char*)szOut);
			gDLL->getEngineIFace()->SaveGame(nameSave, SAVEGAME_AUTO);
			//SAVE
		...
		if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canDoEvent((EventTypes)kTrigger.getEvent(i), *pTriggeredData))
		{
			...
			if (GC.getLogging())
			{
				if(kActivePlayer.isHuman())
				{
					...
					sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
					TCHAR szOut[1024];
					sprintf(szOut, "\t### BUTTON EVENT ### EVENT ID %d | %ls \n\n", kTrigger.getEvent(i), GC.getEventInfo((EventTypes)kTrigger.getEvent(i)).getDescription());
					gDLL->messageControlLog(szOut);
					gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
				}
			}
