...
void CvPlayer::handleDiploEvent(DiploEventTypes eDiploEvent, PlayerTypes ePlayer, int iData1, int iData2)
{
	...
	case DIPLOEVENT_ACCEPT_TAX_RATE:
		...
		if (GC.getLogging())
		{
			...
			sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
			TCHAR szOut[1024];
			sprintf(szOut, "\n### ACCEPT TAX RATE ### Player: %ls increase tax rate by %d. Current Tax Rate: %d \n\n", getName(), iData1, getTaxRate());
			gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
		}
		//
	...
	case DIPLOEVENT_REFUSE_TAX_RATE:
		{
			...
			if (GC.getLogging())
			{
				...
				sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
				TCHAR szOut[1024];
				sprintf(szOut, "\n### REFUSE TAX RATE ### King blocked yield: %ls for Player: %ls\n\n", GC.getYieldInfo((YieldTypes)eYield).getDescription(), getName());
				gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
			}
			//
...
void CvPlayer::disband(CvCity* pCity)
{
	...
	if (GC.getLogging())
	{
		if(isHuman())
		{
			...
			sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
			TCHAR szOut[1024];
			sprintf(szOut, "\n### DESTROY CITY ### Player: %ls CITY: %ls \n\n", getName(), pCity->getName().GetCString());
			gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
		}
	}
	//
