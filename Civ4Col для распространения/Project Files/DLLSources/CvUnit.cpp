...
void CvUnit::kill(bool bDelay, CvUnit* pAttacker)
{
	...
	if(GC.getLogging())
	{
		...
		TCHAR szOut[1024];
		sprintf(szOut, "\n## DEATH UNIT %ls ## Owner: %ls\n\n", getName().GetCString(), GET_PLAYER(getOwner()).getName());
...
void CvUnit::gift(bool bTestTransport)
{
	...
	if (GC.getLogging())
	{
		...
		sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
		TCHAR szOut[1024];
		sprintf(szOut, "###  GIFT UNIT  ###: The player %ls gifted unit %ls to the %ls \n", GET_PLAYER(getOwner()).getName(), pGiftUnit->getName().GetCString(), GET_PLAYER(pPlot->getOwner()).getName());
...
void CvUnit::setProfession(ProfessionTypes eProfession, bool bForce)
{
	...
	if (getProfession() != eProfession)
	{
		...
		if (GC.getLogging())
		{
			...
			{
				if(eProfession != -1 && eOldProfession != -1)
				{
					if(getX() < -100 && getY() < -100)
					{
						...
						sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
						TCHAR szOut[1024];
						sprintf(szOut, "\n### Player: %ls. CHANGE PROFESSION IN EUROPE ### Unit: %ls | OLD_Profession: %ls NEW_Profession: %ls \n\n", GET_PLAYER(getOwner()).getName(), getName().GetCString(), GC.getProfessionInfo((ProfessionTypes)eOldProfession).getDescription(), GC.getProfessionInfo((ProfessionTypes)eProfession).getDescription());
						gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
					} else
					{
						CvCity* pCity = plot()->getPlotCity();
						...
						sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
						TCHAR szOut[1024];
						sprintf(szOut, "\n### Player: %ls. CHANGE PROFESSION IN CITY: %ls ### Unit: %ls | OLD_Profession: %ls NEW_Profession: %ls \n\n", GET_PLAYER(getOwner()).getName(), pCity->getName().GetCString(), getName().GetCString(), GC.getProfessionInfo((ProfessionTypes)eOldProfession).getDescription(), GC.getProfessionInfo((ProfessionTypes)eProfession).getDescription());
						gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
					}
				}
			}
...
void CvUnit::setUnitTravelState(UnitTravelStates eState, bool bShowEuropeScreen)
{
	...
	if (GC.getLogging())
	{
		if(GET_PLAYER(getOwner()).isHuman())
		{
			...
			sprintf(buffer2, "%ls.txt", GC.getInitCore().getGameName().GetCString());
			TCHAR szOut[1024];
			sprintf(szOut, "\n### TRAVEL ### %ls. The ship %ls set off for: %s \n\n", GET_PLAYER(getOwner()).getName(), getName().GetCString(), getTravelState(eState));
			gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
		}
	}
	//
...
const char* CvUnit::getTravelState(UnitTravelStates eState)
{
	switch(eState)
	{
	case NO_UNIT_TRAVEL_STATE: return "NO_UNIT_TRAVEL_STATE";
	case UNIT_TRAVEL_STATE_TO_EUROPE: return "UNIT_TRAVEL_STATE_TO_EUROPE";
	case UNIT_TRAVEL_STATE_IN_EUROPE: return "UNIT_TRAVEL_STATE_IN_EUROPE";
	case UNIT_TRAVEL_STATE_FROM_EUROPE: return "UNIT_TRAVEL_STATE_FROM_EUROPE";
	case UNIT_TRAVEL_STATE_LIVE_AMONG_NATIVES: return "UNIT_TRAVEL_STATE_LIVE_AMONG_NATIVES";
	case UNIT_TRAVEL_STATE_TO_AFRICA: return "UNIT_TRAVEL_STATE_TO_AFRICA";
	case UNIT_TRAVEL_STATE_IN_AFRICA: return "UNIT_TRAVEL_STATE_IN_AFRICA";
	case UNIT_TRAVEL_STATE_FROM_AFRICA: return "UNIT_TRAVEL_STATE_FROM_AFRICA";
	case UNIT_TRAVEL_STATE_TO_PORT_ROYAL: return "UNIT_TRAVEL_STATE_TO_PORT_ROYAL";
	case UNIT_TRAVEL_STATE_IN_PORT_ROYAL: return "UNIT_TRAVEL_STATE_IN_PORT_ROYAL";
	case UNIT_TRAVEL_STATE_FROM_PORT_ROYAL: return "UNIT_TRAVEL_STATE_FROM_PORT_ROYAL";
