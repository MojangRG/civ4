	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		if (kTeamPlayer.isAlive() && kTeamPlayer.getTeam() == getID())
		{
			if (kTeamPlayer.getParent() != NO_PLAYER)
			{
				...
				std::map<int, int> numUnits;
				int numKingUnits = kTeamPlayer.getNumRevolutionEuropeUnits();
				...
				for (int i = 0; i < kTeamPlayer.getNumRevolutionEuropeUnits(); ++i)
				{
					...
					if(numUnits.find(kTeamPlayer.getRevolutionEuropeUnit(i)) != numUnits.end())
					{
						numUnits[kTeamPlayer.getRevolutionEuropeUnit(i)] += 1;
					} else
					{
						numUnits[kTeamPlayer.getRevolutionEuropeUnit(i)] = 1;
					}
				...
				if (GC.getLogging())
				{
					...
					sprintf(szOut, "### NUM REVOLUTION UNITS ###: King %ls have units: %d \n", GET_PLAYER(kTeamPlayer.getParent()).getName(), numKingUnits);
					gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);

					for (std::map<int, int>::iterator iter = numUnits.begin(); iter != numUnits.end(); ++iter) {
						sprintf(szOut, "\n %ls: %d \n", GC.getUnitInfo((UnitTypes)iter->first).getDescription(), iter->second);
						gDLL->logMsg((TCHAR*)buffer2,szOut, false, false);
					}
				...
				sprintf(szOut, "C:\\Users\\%s\\Documents\\My Games\\Civ4Col\\Saves\\multi\\%ls %ls Ð%ls.ColonizationSave", usernameBuffer, GET_PLAYER((PlayerTypes)kTeamPlayer.getID()).getName(), GC.getLeaderHeadInfo(GET_PLAYER((PlayerTypes)kTeamPlayer.getID()).getLeaderType()).getDescription(), szYearStr.GetCString());
